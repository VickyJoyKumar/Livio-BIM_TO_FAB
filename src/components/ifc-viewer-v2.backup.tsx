...
export default function IfcViewerV2({ modelUrl, format, panelId, panelName, onBack }: ViewerProps) {
  ...
  // --- Load model using @thatopen/components for IFC, GLTFLoader for glTF ---
  const loadModel = async () => {
    try {
      if (format === "ifc") {
        await loadIfcWithComponents();
      } else {
        await loadGltf();
      }
      setModelLoaded(true);
    } catch (err) {
      errorRef.current = `Failed to load: ${(err as Error).message}`;
      setLoading(false);
    }
  };
  ...
  const placeMarker = (point: THREE.Vector3, color: number) => {
    ...
    if (!scene) return;
    const sphere = new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 12), new THREE.MeshBasicMaterial({ color }));
  ...
  const makeTextSprite = (text: string): THREE.Sprite => {
    ...
    ctx.beginPath();
    ctx.roundRect?.(10, 10, 236, 44, 8) ?? ctx.fillRect(10, 10, 236, 44);
    ctx.fill();
...
function ToolButton({ icon, label, onClick, active }: { icon: string; label: string; onClick: () => void; active?: boolean }) {
  return (
    <button
      onClick={onClick}
      title={label}
